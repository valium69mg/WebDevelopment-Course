...
app.get("/random", (req,res) => {
  let randIndex = getRandomInt(jokes.length - 1);
...
app.put("/jokes/:id", (req,res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === putId);
  jokes[searchIndex] = putJoke;
  res.json(putJoke);
})
//6. PATCH a joke
...
app.patch("/jokes/:id",(req,res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id === patchId);
  const patchJoke =
  {
    id: patchId,
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === patchId);
